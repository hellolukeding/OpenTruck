...
export function AdminWalletCatalogPanel({
  wallet,
  copy,
  formsCopy,
}: {
  ...
  return (
    <>
      <AdminWalletChannelList channels={wallet?.payment_channels ?? []} copy={copy} formsCopy={formsCopy} />
        ...
        <div className="flex items-center justify-between border-b border-outline-variant/10 px-5 py-4">
          <div>
            <div className="flex items-center gap-3">
              <h3 className="text-[1.2rem] font-semibold text-on-surface">{copy.subscriptionTitle}</h3>
            </div>
            <p className="mt-1 text-[0.88rem] text-on-surface-variant">{copy.subscriptionSubtitle}</p>
          </div>
          <button className="rounded-full border border-outline-variant/20 px-4 py-2 text-[0.82rem] text-on-surface">{copy.subscriptionAction}</button>
        </div>
        <div className="space-y-5 px-5 py-5">
          {wallet && wallet.payment_plans.length > 0 ? (
            <AdminWalletPlanList plans={wallet.payment_plans} copy={copy} formsCopy={formsCopy} />
          ) : (
            <div className="py-12 text-center text-[1rem] text-on-surface-variant">{copy.emptyPlans}</div>
          )}
          <div className="grid gap-5 xl:grid-cols-2">
            <AdminWalletPlanForm copy={formsCopy} />
            <AdminWalletChannelForm copy={formsCopy} />
          </div>
        </div>
      </div>
    </>
