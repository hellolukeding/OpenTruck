...
export function DeveloperSidebar({
  ...
  const navItems = [
    { label: "Overview", href: `/${locale}/developer`, icon: "dashboard", active: true },
    { label: "API Keys", href: `/${locale}/api-keys`, icon: "vpn_key" },
    { label: "Usage", href: `/${locale}/logs`, icon: "bar_chart" },
    { label: "Billing", href: `/${locale}/wallet`, icon: "payments" },
    { label: "Settings", href: `/${locale}/merchant`, icon: "settings" },
  ];
  const mobileNav = [
    { label: "Overview", href: `/${locale}/developer`, icon: "dashboard", active: true },
    { label: "Keys", href: `/${locale}/api-keys`, icon: "vpn_key" },
    { label: "Usage", href: `/${locale}/logs`, icon: "bar_chart" },
    { label: "Settings", href: `/${locale}/merchant`, icon: "settings" },
  ];
      ...
      <aside className="hidden md:flex flex-col w-64 bg-surface-container-low border-r border-outline-variant/20 sticky top-0 h-screen py-lg z-40">
        ...
        <div className="px-md mt-auto pt-lg border-t border-outline-variant/10">
          <div className="flex items-center gap-3 px-xs">
            ...
            <div className="flex flex-col">
              <span className="font-label-md text-label-md text-on-surface truncate w-32">{tenantName}</span>
              <span className="font-body-sm text-body-sm text-secondary truncate">{planLabel}</span>
            </div>
          </div>
        </div>
      </aside>
