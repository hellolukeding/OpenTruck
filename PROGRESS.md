...
- 已新增 `frontend/lib/wallet-page-copy.ts` 与 `wallet-page-copy-types.ts`，将 Wallet 页的页面文案与类型定义独立出来，便于后续继续统一新增控制台页的多语言结构
...
- 已新增 `frontend/lib/tickets-page-copy.ts`，将 Tickets 页的标题、筛选、表单、支持说明、空态和线程详情文案统一收口
- 已将 Tickets 页的状态、优先级、作者类型显示文案以及消息时间格式切到 `en / zh-CN` 双语输出
- 已通过 `make check` 与 `pnpm --dir frontend build` 验证 Tickets 页文案抽离与双语输出全部通过
...
- 已将总览页分析面板 tabs、图表标题、总计文案和空态提示并入 `overview-page-copy.ts`
- 已将总览分析面板的消耗分布/趋势、调用次数分布/排行等文案切到 `en / zh-CN` 双语输出
- 已通过 `make check` 与 `pnpm --dir frontend build` 验证总览分析面板文案抽离全部通过
- 已新增 `frontend/lib/upstream-accounts-page-copy.ts`，将上游账号页的摘要卡片、表格说明和共享提示文案统一收口
- 已将 `upstream_accounts` 页的账号池说明、摘要卡、租户视角、高频错误和 refresh token 提示切到 `en / zh-CN` 双语输出
- 已通过 `make check` 与 `pnpm --dir frontend build` 验证上游账号页文案抽离全部通过
- 已将 `upstream_accounts` 页的 OAuth intake 双步骤表单、详情/编辑/刷新/删除 dialog 和相关成功提示并入统一 copy
- 已将上游账号页的 OAuth 授权链接生成、账号接入、更新、刷新和删除反馈切到 `en / zh-CN` 双语输出
- 已将超长 `upstream-accounts-page-copy.ts` 拆分为 `types + en + zh + 入口` 四个文件，继续满足前端单文件不超过 300 行的约束
- 已通过 `make check` 与 `pnpm --dir frontend build` 验证上游账号页剩余共享提示与操作反馈文案抽离全部通过
...
5. 继续将新增页面的多语言文案抽离并整合到统一字典中，优先处理剩余资源页行操作文案与创建表单反馈
