...
- 已新增 `payment_plans` 与 `payment_channels` 两张钱包目录表，并通过 `20260515_0006_wallet_catalog` 迁移写入首批套餐和支付渠道种子数据
- 已将 `/admin/wallet` 扩展为返回真实套餐目录与支付渠道配置，钱包页不再依赖硬编码的充值档位和渠道文案
- 已将钱包页拆出套餐卡片与渠道列表组件，展示后端驱动的充值目录，同时继续保持文件行数约束
- 已新增 `/admin/payment-plans` 与 `/admin/payment-channels` 两组后端 CRUD 接口，为钱包目录管理提供正式 API 入口
- 已在钱包页新增最小可用的套餐/支付渠道创建表单，支持从控制台直接新增目录数据
