...
- 按当前产品反馈，首页登录入口优先改为可关闭弹窗；同时补一个环境变量驱动的 operator 账号密码入口，改善本地和早期部署体验，但暂不引入完整用户密码体系
- 未配置显式 operator 凭据时，仅在 `localhost / 127.0.0.1 / 私有网段` 预览中暴露默认本地凭据回退，避免生产公网部署意外暴露默认账号
